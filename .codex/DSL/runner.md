...
- Codex can only execute tasks in `.codex/DSL/tasks/` with status `READY`.
...
- Task generation must follow the relevant flow in `.codex/DSL/flows/gen/` (and the rules in `.codex/DSL/rules/01-task-gen-rules.adoc`).
...
- Codex 只能执行 `.codex/DSL/tasks/` 目录下状态为 `READY` 的任务。
...
- 任务生成必须遵循 `.codex/DSL/flows/gen/` 中对应的流程，并受 `.codex/DSL/rules/01-task-gen-rules.adoc` 约束。
